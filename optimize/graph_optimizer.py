...
class OnnxGraphOptimizer:
    ...
    def optimize(self, model_path, output_path, hardware_profile=None):
        ...
        try:
            ...
            original_size = self._model_size_bytes(
                model_path
            )
            ...
            transformer_stats = (
                self._try_transformer_optimizer(
                    model_path,
                    output_path,
                    hardware_profile,
                )
            ...
            if transformer_stats["applied"]:
                passes_applied.extend(
                    transformer_stats["passes_applied"]
                )
            ...
            optimized_model = self._validate_graph(
                onnx,
                output_path,
            )

            optimized_node_count = len(
                optimized_model.graph.node
            )

            optimized_size = self._model_size_bytes(
                output_path
            )
            ...
            logger.exception(
                "ONNX graph optimization failed: %s",
                exc,
            )
            ...
            raise GraphOptimizationError(
                f"Failed to optimize ONNX graph "
                f"'{model_path}'"
            ) from exc
    ...
    def _try_transformer_optimizer(
        ...
        model_type = hardware_profile.get(
            "graph_optimization_model_type"
        )
        ...
        if not model_type:
            return {
                "applied": False,
                "reason":
                    "No graph_optimization_model_type configured",
                "passes_applied": [],
            }

        try:
            from onnxruntime.transformers.optimizer import (
                optimize_model,
            )

        except ImportError as exc:
            raise GraphOptimizationError(
                "onnxruntime transformer optimizer "
                "APIs are unavailable"
            ) from exc

        logger.info(
            "Using ONNX Runtime transformer optimizer "
            "for model type: %s",
            model_type,
        )
        ...
        optimized_model = optimize_model(
            ...
            num_heads=hardware_profile.get(
                "num_heads",
                0,
            ),
            hidden_size=hardware_profile.get(
                "hidden_size",
                0,
            ),
            opt_level=hardware_profile.get(
    ...
    def _run_onnxruntime_graph_optimization(
        self,
        model_path,
        output_path,
    ):
        ...
        session_options.graph_optimization_level = (
            self._ort_optimization_level(ort)
        )
    ...
    def _run_guarded_onnxruntime_graph_optimization(
        ...
        candidate_path = output_path.with_name(
            f"{output_path.stem}.candidate"
            f"{output_path.suffix}"
        )

        retry_path = output_path.with_name(
            f"{output_path.stem}.extended_candidate"
            f"{output_path.suffix}"
        )
        ...
        selected_label = (
            "onnxruntime_optimized_graph"
        )

        passes = (
            self._run_onnxruntime_graph_optimization(
                model_path,
                candidate_path,
            )
        )
        ...
        candidate_increase = (
            self._size_increase_percent(
                original_size,
                self._model_size_bytes(
                    candidate_path
                ),
            )
        )
        ...
        if (
            candidate_increase
            > self.max_size_increase_percent
        ):

            logger.warning(
                "Graph optimization increased graph "
                "size by %.2f%%",
                candidate_increase,
            )
            ...
            if self.optimization_level == "all":
                ...
                if retry_increase < candidate_increase:
                    ...
                    selected_label = (
                        "onnxruntime_extended_optimized_graph"
                    )
            ...
            if (
                ...
                logger.warning(
                    "Graph optimization artifact "
                    "remains inflated by %.2f%%",
                    candidate_increase,
                )
                ...
                selected_label = (
                    "original_copy_due_to_size_bloat"
                )
        ...
        return passes + [
            f"bloat_guard_threshold_percent:"
            f"{self.max_size_increase_percent}",
            f"selected_artifact:{selected_label}",
        ]

    def _size_increase_percent(
        self,
        original_size,
        new_size,
    ):
    ...
    def _ort_optimization_level(self, ort):
        """
        Use safe ONNX Runtime optimization levels.

        IMPORTANT:
        Avoid ORT_ENABLE_ALL because it enables
        CPU-specific NCHWc transforms that can
        break FP16 graphs for Jetson/TensorRT.
        """
        ...
        if self.optimization_level not in levels:
            raise GraphOptimizationError(
                f"Unsupported graph optimization "
                f"level: {self.optimization_level}"
            )

        logger.info(
            "Using ONNX Runtime graph "
    ...
    def _build_stats(
        ...
        return {
            ...
            "node_count_reduction":
                node_reduction,
            "node_count_reduction_percent":
                round(node_reduction_percent, 2),
            "original_size_bytes":
                original_size,
